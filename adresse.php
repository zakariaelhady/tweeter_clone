...
$adresse=$_POST["email"];
    $mail_update="UPDATE user SET email='$adresse' where user_id='$id'";
    $user_query=mysqli_query($db,$mail_update);
    if ($user_query) {
    echo " Votre email a été modifié correctement";
...
<html lang="fr" dir="ltr">
    <head>
        ...
        article a:hover:nth-child(3)
        {
          background: linear-gradient(90deg,#EAC4D5,#B58DB6);
        }

        article a:hover:nth-child(4)
        {
          background: linear-gradient(90deg,#B58DB6,#EAC4D5);
        }
        article a:hover:nth-child(5)
        {
          background: linear-gradient(90deg,#EAC4D5,#B58DB6);
        }
        article{
    ...
    <body>
        ...
        <header>
          ...
          <h2>Modifier des paramètres:</h2>
        </header>
        <article>
          <form class="modif" action="#" method="post">
            <div class="txt">
              <label>Adresse mail</label><br>
              <input type="email"  name="email" placeholder="Adresse mail" size="80" value="<?php echo $result['email']; ?>" />
