...
if (isset($_POST["modif"])) {$mdp=$_POST["mdp"];
if ($mdp) {
  if (strlen($mdp)>=8) {
    $mdp_update="UPDATE user SET password='$mdp' where user_id='$id'";
    $user_query=mysqli_query($db,$mdp_update);
    if ($user_query) {
        ...
        <article>
          <form class="modif" action="#" method="post">
            <div class="motdepasse">
              <input type="text" name="mdp" maxlength="30" value="<?php echo $result["password"]; ?>">
